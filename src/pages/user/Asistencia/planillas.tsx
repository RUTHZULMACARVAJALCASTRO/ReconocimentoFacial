...
const Planillas = () => {
    ...
    const procesarDatos = async (data: PersonalData[]) => {
        ...
        const promises = data.map(async personal => {
            ...
            const horario = await axios.get(`http://10.10.214.24:3000/api/schedule/${personal.schedule}`);
    ...
    const fetchData = async () => {
        try {
            const response = await axios.get(`http://10.10.214.24:3000/api/attendance/`);
