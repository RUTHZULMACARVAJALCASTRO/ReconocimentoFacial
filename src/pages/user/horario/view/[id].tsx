...
const PlanillaHorario: React.FC<PlanillaHorarioProps> = ({ scheduleId }) => {
    ...
    const fetchData = async () => {
        try {
            const { data } = await axios.get(
                `https://cold-cameras-stop.loca.lt/api/schedule/${scheduleId}`
            );
            ...
            const scheduleSpecialData = data.scheduleSpecial.map((schedule: ScheduleSpecial, index: number) => ({
                ...
                usersAssigned: schedule.usersAssigned.join(', '),
            }));
