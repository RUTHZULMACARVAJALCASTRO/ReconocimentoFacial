...
const SidebarEditLicense = ({ licenseId, open, toggle }: SidebarEditLicenseType) => {
    ...
    const { handleSubmit, control, setValue } = useForm<LicenseData>({
        ...
        resolver: yupResolver(
            yup.object().shape({
                // Agrega las validaciones necesarias aquí
                licenseType: yup.string().required('El tipo de licencia es obligatorio'),
    ...
    const onSubmit = async (data: LicenseData) => {
        try {
            if (selectedUserId !== null) {
                data.personal = selectedUserId;
            } else {
                // Manejar el caso en el que selectedUserId sea null, por ejemplo, mostrando un mensaje de error o tomando otra acción apropiada.
                // Por ejemplo, puedes mostrar una alerta:
                MySwal.fire({
                    title: 'Error',
                    text: 'Debe seleccionar un usuario válido.',
                    icon: 'error',
                });
                return; // Salir de la función onSubmit para evitar enviar datos incorrectos
            }
